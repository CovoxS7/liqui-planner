...
const eingabeformular = {
    ...
    eintraege_verarbeiten(formulardaten) {
        return {
    ...
    absenden_event_hinzufügen(eingabeformular) {
        eingabeformular.querySelector("#eingabeformular").addEventListener("submit", e => {
            e.preventDefault();
            let formulardaten = this.eintraege_verarbeiten(this.formulardaten_holen(e));
            let formular_fehler = this.formulardaten_validieren(formulardaten);
            if (formular_fehler.length === 0) {
                haushaltsbuch.eintrag_hinzufügen(formulardaten);
                this.bestehende_fehlerbox_entfernen();
                e.target.reset();
    ...
    bestehende_fehlerbox_entfernen() {
